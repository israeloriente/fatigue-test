...
void loop()
{
  ...
  { // O programa inicia com o dado da tensão de flexão do teste via porta serial, executa esse loop até o usuário informar o valor da tensão
    delay(10);
    ...
    while (t < tset)
    { // Após a leitura da tração do cabo, a máquina entra no loop abaixo até que a tração do cabo atinja a tração desejada.

      for (int i = 0; i < 1000; i++)
      { // Executa mil ciclos no motor de passo (Cada ciclo corresponte ao movimento de 1,8º)
        pulso = !pulso;
    ...
    while (t > tset / 2)
    { // Enquanto a tração do cabo for maior que a tração definida/2, o loop abaixo mantem o motor de passo principal acionado
